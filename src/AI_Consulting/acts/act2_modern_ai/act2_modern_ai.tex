...
\begin{frame}{Building Block 2: Vector Search}
    ...
    \vspace{0.3em}
    \begin{theorembox}{vs. Keyword Search}
        \textbf{Keyword:} "vacation policy" finds docs with those exact words \\
        \textbf{Vector:} "vacation policy" also finds "PTO rules", "time off", "leave entitlement"
    \end{theorembox}
\end{frame}
...
\begin{frame}{Putting It Together: The RAG Flow}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.85, every node/.style={font=\small}]
            % User query
            \node[draw, fill=TuebingenBeige, rounded corners, minimum width=3cm] (query) at (0, 3) {User: "What's our PTO policy?"};
            
            % Step 1: Embed query
            \node[draw, fill=TuebingenRot!20, rounded corners, minimum width=2.5cm] (embed) at (0, 1.5) {1. Embed Query};
            \node[font=\tiny, TuebingenGray, right] at (1.5, 1.5) {[0.23, -0.41, ...]};
            
            % Step 2: Vector search
            \node[draw, fill=TuebingenGold!20, rounded corners, minimum width=2.5cm] (search) at (0, 0) {2. Vector Search};
            
            % Vector DB
            \node[draw, fill=TuebingenGray!20, rounded corners, minimum width=2cm] (db) at (4, 0) {Vector DB};
            
            % Step 3: Retrieved docs
            \node[draw, fill=TuebingenGold!40, rounded corners, minimum width=3cm, align=center] (docs) at (4, 1.5) {Retrieved Docs\\{\tiny HR-Policy-PTO.pdf}};
            
            % Step 4: LLM
            \node[draw, fill=TuebingenGreen!20, rounded corners, minimum width=2.5cm] (llm) at (4, 3) {3. LLM Generate};
            
            % Answer
            \node[draw, fill=TuebingenCyan!30, rounded corners, minimum width=5cm, align=center] (answer) at (2, -1.5) {Answer: "You receive 20 days PTO per year.\\ See HR-Policy-PTO.pdf §3.1"};
            ...
            \draw[->, thick] (db) -- (docs);
            \draw[->, thick] (docs) -- (llm);
            \draw[->, thick] (query.east) -- ++(0.5,0) |- (llm.west);
    ...
    \vspace{0.3em}
    \begin{center}
        \small\color{TuebingenGray} This is the core RAG pattern. Now let's explore each step in detail.
    \end{center}
\end{frame}
...
\begin{frame}{RAG Roadmap: What's Next}
    ...
    \vspace{0.5em}
    \begin{theorembox}{Learning Goal}
        By the end of this section, you'll understand: \\
        • How to design a RAG system for your use case \\
        • Which variants apply to which problems \\
...
\begin{frame}{Enterprise RAG: Why Audit Matters}
    \framesubtitle{Business value of traceability}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \setlength\itemsep{0.6em}
            \item \textbf{Compliance:} Demonstrate data handling
            \item \textbf{Investigation:} Trace problematic answers
            \item \textbf{Quality debugging:} Why did it fail?
        \end{itemize}
        
        \column{0.48\textwidth}
        \begin{itemize}
            \setlength\itemsep{0.6em}
            \item \textbf{Improvement:} Build evaluation datasets
            \item \textbf{Cost tracking:} Token usage per query
            \item \textbf{User feedback:} Connect ratings to queries
        \end{itemize}
    \end{columns}
    
    \vspace{0.5em}
    {\color{TuebingenGray}\textbf{Implementation:} Use structured logging with query IDs that link retrieval, generation, and user feedback.}
\end{frame}

\begin{frame}{Evaluating RAG: Retrieval Metrics}
    \framesubtitle{Did we find the right documents?}
    
    \vspace{0.5em}
    \textbf{Key Retrieval Metrics:}
    
    \vspace{0.3em}
    \begin{itemize}
        \setlength\itemsep{0.6em}
        \item \textbf{Recall@k:} Did we retrieve the right docs in top k results?
        \item \textbf{Precision@k:} How much noise/irrelevant content in top-k?
        \item \textbf{MRR (Mean Reciprocal Rank):} Where does first relevant doc appear?
        \item \textbf{nDCG:} Overall ranking quality considering position
    \end{itemize}
    
    \vspace{0.5em}
    \begin{theorembox}{Key Insight}
        If retrieval fails, generation can't succeed. \\
        \textbf{Always measure retrieval quality separately from generation quality.}
    \end{theorembox}
\end{frame}

\begin{frame}{Evaluating RAG: Generation Metrics}
    \framesubtitle{Is the answer good?}
    
    \vspace{0.5em}
    \textbf{Key Generation Metrics:}
    
    \vspace{0.3em}
    \begin{itemize}
        \setlength\itemsep{0.6em}
        \item \textbf{Groundedness:} Is answer supported by retrieved sources?
        \item \textbf{Citation accuracy:} Do citations actually match claims?
        \item \textbf{Factuality:} Is the answer correct?
        \item \textbf{Completeness:} Did we fully answer the question?
        \item \textbf{Refusal behavior:} Appropriate "I don't know" when needed?
    \end{itemize}
    
    \vspace{0.5em}
    {\color{TuebingenGray}\textbf{Tip:} Use LLM-as-judge for scalable automated evaluation.}
    
    \vspace{0.3em}
    \begin{center}
        \small\color{TuebingenGold} \textbf{Diagnose failures:} Poor answer = retrieval problem OR generation problem. Metrics tell you which.
    \end{center}
\end{frame}

\begin{frame}{Evaluating RAG: Building Your Test Set}
    \framesubtitle{Creating a reliable evaluation foundation}
    ...
    \begin{enumerate}
        \setlength\itemsep{0.6em}
        \item \textbf{Collect} realistic queries from actual users
        \item \textbf{Identify} authoritative source for each answer
        \item \textbf{Create} "golden answers" with expert review
        \item \textbf{Track} which sources should be retrieved
        \item \textbf{Version} the eval set as docs change
    \end{enumerate}
    
    \vspace{0.5em}
    {\color{TuebingenGray}\textbf{Target:} 100+ representative queries covering edge cases and common requests.}
\end{frame}

\begin{frame}{Evaluating RAG: Evaluation Cadence}
    \framesubtitle{When and how to evaluate}
    
    \vspace{0.5em}
    \textbf{Recommended Cadence:}
    
    \vspace{0.3em}
    \begin{itemize}
        \setlength\itemsep{0.6em}
        \item \textbf{Automated:} Run on every change to retrieval or prompts
        \item \textbf{Human spot-check:} Weekly sample review of outputs
        \item \textbf{Deep dive:} Monthly comprehensive audit
        \item \textbf{User feedback:} Continuous collection and analysis
    \end{itemize}
    
    \vspace{0.5em}
    \begin{theorembox}{Evaluation Governance}
        Treat your evaluation set like code: \textbf{version controlled}, \textbf{reviewed}, and \textbf{protected}. \\
        A contaminated eval set gives false confidence.
    \end{theorembox}
\end{frame}
...
\begin{frame}{RAG: Key Takeaways}
    \framesubtitle{Summary for decision-makers}
    ...
    \begin{itemize}
        ...
        \item Enables \textbf{citations} and \textbf{audit trails}
    ...
    \begin{itemize}
        \setlength\itemsep{0.4em}
        \item Start with \textbf{hybrid retrieval} (vector + keyword)
        \item Add \textbf{reranking} for precision
        \item Use \textbf{hierarchical chunking} for large document sets
        \item Add \textbf{GraphRAG} for complex relationships
        \item Use \textbf{Text-to-SQL} for structured data
    \end{itemize}
\end{frame}

\begin{frame}{RAG: Success Criteria}
    \framesubtitle{Non-negotiables for production}
    
    \vspace{0.5em}
    \textbf{Non-Negotiable Requirements:}
    \begin{itemize}
        \setlength\itemsep{0.5em}
        \item Permission enforcement at retrieval time
    ...
    \vspace{0.5em}
    \textbf{Questions to Answer "Yes" To:}
    \begin{itemize}
        \setlength\itemsep{0.4em}
...
\begin{frame}{Production Monitoring: What to Track}
    \framesubtitle{Deployment is just the beginning}
    
    \vspace{0.5em}
    \textbf{Key Monitoring Dimensions:}
    \begin{itemize}
        \setlength\itemsep{0.5em}
        \item \textbf{Input drift:} Are user queries changing over time?
        \item \textbf{Output quality:} Hallucination rate, inappropriate refusals
        \item \textbf{Retrieval health:} Empty results, low confidence scores
        \item \textbf{Latency:} Response time percentiles (p50, p95, p99)
        \item \textbf{Cost:} Tokens used, API spend per query
        \item \textbf{Security:} Injection attempts, PII exposure
    \end{itemize}
    ...
    \begin{itemize}
        \setlength\itemsep{0.4em}
        \item User thumbs up/down on responses
        \item Escalation to human operators
        \item Correction submissions from users
    \end{itemize}
\end{frame}

...
\begin{frame}{t-SNE: Critical Warnings for Executives}
    ...
    \framesource{van der Maaten \& Hinton, 2008}
...
\begin{frame}{Autoencoders: Learning to Compress}
    ...
    \begin{columns}[T]
        ...
        \begin{center}
            \begin{tikzpicture}[scale=0.85]
                % Input
                \draw[fill=TuebingenBeige, draw=TuebingenAnthrazit] (0,0) rectangle (1,2.5);
                \node[font=\scriptsize, rotate=90] at (0.5, 1.25) {Input (high-dim)};
                
                % Encoder arrow
                \draw[->, very thick, TuebingenRot] (1.2, 1.25) -- (2.3, 1.25);
                \node[font=\tiny, above] at (1.75, 1.35) {Encoder};
                
                % Latent (bottleneck)
                \draw[fill=TuebingenGold!40, draw=TuebingenGold, thick] (2.5,0.7) rectangle (3,1.8);
                \node[font=\scriptsize, rotate=90] at (2.75, 1.25) {Latent};
                
                % Decoder arrow
                \draw[->, very thick, TuebingenGreen] (3.2, 1.25) -- (4.3, 1.25);
                \node[font=\tiny, above] at (3.75, 1.35) {Decoder};
                
                % Output
                \draw[fill=TuebingenBeige, draw=TuebingenAnthrazit] (4.5,0) rectangle (5.5,2.5);
                \node[font=\scriptsize, rotate=90] at (5, 1.25) {Output (reconstruction)};
                
                % Loss
                \draw[<->, dashed, TuebingenRot] (0.5, -0.3) -- (5, -0.3);
                \node[font=\tiny, below] at (2.75, -0.4) {Minimize reconstruction error};
            \end{tikzpicture}
        \end{center}
    ...
    \begin{columns}[T]
        ...
        \begin{center}
            \begin{tikzpicture}[scale=0.8]
                % Time series plot
                \draw[->] (0,0) -- (5.5,0) node[right, font=\tiny] {Time};
                \draw[->] (0,0) -- (0,3) node[above, font=\tiny] {Recon. Error};
                
                % Normal region
                \draw[TuebingenGreen, thick] (0.2, 0.5) -- (0.5, 0.6) -- (0.8, 0.4) -- (1.1, 0.5) -- (1.4, 0.55) -- (1.7, 0.45) -- (2, 0.5) -- (2.3, 0.6);
                
                % Anomaly buildup
                \draw[TuebingenGold, thick] (2.3, 0.6) -- (2.6, 0.7) -- (2.9, 0.9) -- (3.2, 1.2) -- (3.5, 1.5);
                
                % Alert region
                \draw[TuebingenRot, very thick] (3.5, 1.5) -- (3.8, 1.9) -- (4.1, 2.3) -- (4.4, 2.5);
                
                % Threshold line
                \draw[dashed, TuebingenRot] (0, 1.3) -- (5.2, 1.3);
                \node[font=\tiny, TuebingenRot, right] at (5.2, 1.3) {Threshold};
                
                % Labels
                \node[font=\tiny, TuebingenGreen] at (1.2, 1) {Normal};
                \node[font=\tiny, TuebingenRot] at (4, 2.8) {Alert!};
                
                % Failure marker
                \draw[TuebingenRot, very thick] (4.8, 0) -- (4.8, 2.7);
                \node[font=\tiny, TuebingenRot] at (4.8, -0.3) {Failure};
            \end{tikzpicture}
        \end{center}
        
        \vspace{0.3em}
        {\small\color{TuebingenGray} \textbf{Result:} Days of early warning before failure, enabling preventive maintenance.}
    \end{columns}
    
    \vspace{0.3em}
    \begin{center}
        \small\color{TuebingenGold} \textbf{Bridge:} This encoder→latent→decoder pattern is fundamental to modern generative AI.
    \end{center}
\end{frame}
...
\begin{frame}{ML Failure Modes Executives Must Know}
    \framesubtitle{What causes AI projects to fail in production}
    
    \vspace{0.3em}
    \begin{columns}[T]
        \column{0.48\textwidth}
        {\color{TuebingenRot}\textbf{Data Leakage}} — The Silent Killer
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Information from future/test leaks into training
            \item Model appears perfect but fails in production
            \item \textbf{Examples:}
            \begin{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Production data differs from training data
            \item Model degrades over time
            \item \textbf{Causes:} Seasonality, market changes, new user segments
        \end{itemize}
        
        \column{0.48\textwidth}
        {\color{TuebingenCyan}\textbf{Label Quality Issues}}
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Garbage in = garbage out
            \item Inconsistent labeling
            \item Missing or delayed labels
        \end{itemize}
        
        \vspace{0.3em}
        {\color{TuebingenGreen}\textbf{Evaluation Leakage}}
        \begin{itemize}
            \setlength\itemsep{0.3em}
    ...
    \begin{theorembox}{Governance Requirement}
        Mandate \textbf{evaluation governance}: separate teams for model development and final evaluation, strict hold-out sets, documented data lineage, and drift monitoring in production.
    \end{theorembox}
\end{frame}
...
\begin{frame}{Context Window Strategy: Less Is Often More}
    ...
    \begin{columns}[T]
        \column{0.48\textwidth}
        {\color{TuebingenRot}\textbf{Naive Approach:}}\\[0.3em]
        ...
        \column{0.48\textwidth}
        {\color{TuebingenGreen}\textbf{Smart Approach:}}\\[0.3em]
...
\begin{frame}{Tool Use: Extending LLM Capabilities}
    ...
    {\color{TuebingenGray} LLMs can learn to call external tools—calculators, APIs, databases—to overcome their limitations.}
...
\begin{frame}{Agentic Patterns: Bounded Autonomy}
    \framesubtitle{Multi-step reasoning with guardrails}
    ...
    \begin{columns}[T]
        ...
        \column{0.48\textwidth}
        {\color{TuebingenRot}\textbf{Governance Requirements:}}
        \begin{itemize}
            \setlength\itemsep{0.4em}
...
\begin{frame}{Part II-D: RAG Systems in Detail}
    ...
    \begin{enumerate}
        \setlength\itemsep{0.5em}
        \item Why RAG exists — the parametric memory problem
        \item The canonical RAG pipeline — 9 stages
        \item RAG variants — from naive to enterprise-grade
        \item Evaluation — measuring what matters
        \item Example walkthrough — a realistic enterprise query
    \end{enumerate}
...
\begin{frame}{The Canonical RAG Pipeline: Overview}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.8, every node/.style={font=\footnotesize}]
            % Pipeline stages
            \node[draw, fill=TuebingenRot!20, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (ingest) at (0,0) {1. Ingest};
            \node[draw, fill=TuebingenGold!20, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (chunk) at (2,0) {2. Chunk};
            \node[draw, fill=TuebingenGreen!20, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (embed) at (4,0) {3. Embed};
            \node[draw, fill=TuebingenCyan!20, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (retrieve) at (6,0) {4. Retrieve};
            \node[draw, fill=TuebingenRot!30, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (rerank) at (8,0) {5. Rerank};
            
            \node[draw, fill=TuebingenGold!30, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (prompt) at (2,-1.5) {6. Prompt};
            \node[draw, fill=TuebingenGreen!30, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (generate) at (4,-1.5) {7. Generate};
            \node[draw, fill=TuebingenCyan!30, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (post) at (6,-1.5) {8. Post-proc};
            \node[draw, fill=TuebingenAnthrazit!20, rounded corners, minimum width=1.4cm, minimum height=0.7cm] (feedback) at (8,-1.5) {9. Feedback};
            ...
            \draw[->, thick] (rerank) -- (8, -0.75) -- (prompt);
            \draw[->, thick] (prompt) -- (generate);
            \draw[->, thick] (generate) -- (post);
            \draw[->, thick] (post) -- (feedback);
            
            % Feedback loop
            \draw[->, dashed, TuebingenGray] (feedback) -- (8, -2.5) -- (0, -2.5) -- (ingest);
        \end{tikzpicture}
    \end{center}
...
\begin{frame}{RAG Pipeline: Embedding, Retrieval, Reranking}
    \framesubtitle{Finding the needle in the haystack}
    
    \vspace{0.3em}
    \begin{columns}[T]
        \column{0.32\textwidth}
        \textbf{3. Embedding + Indexing:}
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Convert chunks to vectors
            \item Store in vector index
            \item Enable similarity search
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item Dedicated vector DB
            \item Vector extension in existing DB
            \item Hybrid with keyword index
        \end{itemize}
        ...
        \vspace{0.3em}
        {\color{TuebingenRot}\textbf{Recall vs Precision:}}
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item High k → more recall, more noise
            \item Low k → might miss relevant info
            \item Balance via reranking
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Take top-N candidates
            \item Score with cross-encoder
            \item Return top-K highest
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item Reranker = slower but precise
            \item Retrieve 50 → Rerank to 5
        \end{itemize}
    \end{columns}
...
\begin{frame}{RAG Pipeline: Generation \& Post-Processing}
    \framesubtitle{From context to trustworthy answer}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item System instructions (persona, constraints)
            \item Retrieved context (with source markers)
            \item User query
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item LLM produces answer using context
            \item \textbf{Key instruction:} "Only use provided context"
            \item Citation markers: [Source 1], [Doc A]
            \item Refusal when uncertain or no relevant context
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Log query, retrieval, response
            \item Capture user feedback (thumbs up/down)
            \item Build evaluation dataset
            \item Identify retrieval failures
        \end{itemize}
    \end{columns}
    
    \vspace{0.5em}
    {\color{TuebingenGray}\textbf{Executive insight:} The feedback loop is how RAG systems \textbf{improve over time}. Without it, you're flying blind.}
\end{frame}
...
\begin{frame}{Enterprise RAG: Permissions \& Audit}
    \framesubtitle{The governance layer that makes RAG safe}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item RAG can expose \textbf{unauthorized data}
            \item "Summarize all HR docs" → returns confidential info
            \item UI-level permissions are \textbf{not enough}
        \end{itemize}
        
        \vspace{0.3em}
        \textbf{Permission Enforcement:}
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item \textbf{At indexing:} Store ACLs with chunks
            \item \textbf{At retrieval:} Filter by user permissions
        ...
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item \textbf{What was retrieved?} Source IDs, chunks
            \item \textbf{What was generated?} Full response
            \item \textbf{Who asked?} User identity
            \item \textbf{When?} Timestamp
        ...
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item Compliance investigation
            \item Quality debugging
...
\begin{frame}{Evaluating RAG Systems}
    \framesubtitle{Measuring what matters}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item \textbf{Recall@k:} Did we retrieve the right docs?
            \item \textbf{Precision@k:} How much noise in top-k?
            \item \textbf{MRR:} Where does first relevant doc appear?
            \item \textbf{nDCG:} Ranking quality
        \end{itemize}
        
        \vspace{0.3em}
        {\color{TuebingenGray}\textbf{Key insight:} If retrieval fails, generation can't succeed. Measure retrieval separately.}
        
        \column{0.48\textwidth}
        \textbf{End-to-End Metrics:}
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item \textbf{Groundedness:} Is answer supported by sources?
            \item \textbf{Citation accuracy:} Do citations match claims?
            \item \textbf{Factuality:} Is answer correct?
            \item \textbf{Completeness:} Did we answer the question?
            \item \textbf{Refusal behavior:} Does it refuse appropriately?
        \end{itemize}
    \end{columns}
    
    \vspace{0.5em}
    \textbf{Building Your Evaluation Set:}
    \begin{enumerate}
        \setlength\itemsep{0.2em}
        \item Collect realistic queries from users
        \item Identify authoritative source for each answer
        \item Create "golden answers" with expert review
        \item Track which sources should be retrieved
        \item Run automated + human evaluation regularly
    \end{enumerate}
\end{frame}

\begin{frame}{RAG Example: Enterprise Policy Query}
    \framesubtitle{A realistic walkthrough}
    
    \vspace{0.3em}
    \textbf{User Query:} "What is our policy for remote work, and who approves exceptions?"
    
    \vspace{0.5em}
    \begin{columns}[T]
        \column{0.48\textwidth}
        \textbf{1. Retrieval Results:}
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item[$\checkmark$] HR-Policy-2024-Remote-Work.pdf (score: 0.92)
            \item[$\checkmark$] Exception-Approval-Process.docx (score: 0.87)
            \item[$\times$] Travel-Policy.pdf (score: 0.71, filtered)
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item Cross-encoder promotes Exception doc
            \item Final context: 2 docs, 4 chunks
        \end{itemize}
        
        \column{0.48\textwidth}
        \textbf{3. Generated Answer:}
        
        {\small "Our remote work policy (HR-2024-RW) allows up to 3 days/week remote for eligible roles. \textbf{[Source 1, §2.1]}
        
        Exceptions require approval from your department head and HR Business Partner. \textbf{[Source 2, §4.2]}
        
        Submit exception requests via ServiceNow."}
        
        \vspace{0.3em}
        \textbf{4. Audit Log:}
        {\footnotesize Query ID, User, Timestamp, Sources used, Confidence scores}
    \end{columns}
    
    \vspace{0.5em}
    {\color{TuebingenGreen}\textbf{Key features:} Citations, specific references, actionable answer, traceable to sources.}
\end{frame}

\begin{frame}{RAG: Executive Summary}
    \framesubtitle{Key takeaways for decision-makers}
    
    \vspace{0.3em}
    \begin{columns}[T]
        \column{0.48\textwidth}
        \textbf{RAG Is The Enterprise Pattern:}
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item Grounds LLMs in \textbf{your} knowledge
            \item Enables \textbf{citations} and \textbf{audit}
            \item Keeps data \textbf{current} without retraining
            \item Respects \textbf{permissions} and \textbf{compliance}
        \end{itemize}
        
        \vspace{0.3em}
        \textbf{Choose Your Variant:}
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Start with \textbf{hybrid retrieval}
            \item Add \textbf{reranking} for precision
            \item Use \textbf{hierarchical} for large doc sets
            \item Add \textbf{GraphRAG} for relationships
            \item Use \textbf{Text-to-SQL} for structured data
        \end{itemize}
        
        \column{0.48\textwidth}
        \textbf{Non-Negotiable Requirements:}
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item Permission enforcement at retrieval
            \item Complete audit logging
            \item Evaluation set and metrics
            \item Feedback loop for improvement
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Can you trace every answer to sources?
            \item Can you measure retrieval quality?
            \item Can you prove permission compliance?
            \item Can you show improvement over time?
        \end{itemize}
    \end{columns}
    
    \vspace{0.5em}
    \begin{theorembox}{The Bottom Line}
        RAG transforms LLMs from \textbf{impressive demos} into \textbf{trustworthy enterprise tools}. \\
        The investment is in the \textbf{retrieval infrastructure}, not the model.
    \end{theorembox}
\end{frame}
...
\begin{frame}{Why Benchmarks Matter}
    ...
    \begin{columns}[T]
        ...
        \column{0.48\textwidth}
        {\color{TuebingenRot}\textbf{Critical Caveat:}}
        
        \vspace{0.5em}
        \begin{center}
            \fbox{\parbox{0.9\linewidth}{\centering Benchmark performance $\neq$ \\ Your business task performance}}
        \end{center}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Your data distribution differs
            \item Your success criteria differ
            \item Your failure costs differ
            \item Benchmark contamination in training
        \end{itemize}
    \end{columns}
    
    \vspace{0.5em}
    {\color{TuebingenGray}\textbf{Executive implication:} Use benchmarks for initial screening, but \textbf{build your own evaluation set} for deployment decisions.}
\end{frame}
...
\begin{frame}{Evaluation Governance}
    \framesubtitle{Process discipline for trustworthy results}
    
    \vspace{0.3em}
    \begin{columns}[T]
        \column{0.48\textwidth}
        \textbf{Who Can See What, When:}
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item \textbf{Training data:} Available to developers
            \item \textbf{Validation data:} Available during development
            \item \textbf{Test data:} \textcolor{TuebingenRot}{Restricted access}
        ...
        \begin{itemize}
            \setlength\itemsep{0.3em}
            \item Repeated test usage → overfitting
            \item Public leaderboards incentivize gaming
            \item Business decisions need unbiased estimates
        \end{itemize}
        ...
        \begin{enumerate}
            ...
            \item Lock it away (separate repo/access)
            \item Develop using train + validation only
            \item Run test evaluation \textbf{once} for go/no-go
            \item Document results, don't iterate
        \end{enumerate}
        ...
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item "Golden set" of Q/A pairs
            \item Expert-validated answers
            \item Versioned and maintained
        \end{itemize}
    \end{columns}
    ...
    \begin{theorembox}{Executive Mandate}
        ...
        Ask: "Who has access to test data? How many times has it been used?"
    \end{theorembox}
\end{frame}

\begin{frame}{Production Monitoring: Because Deployment Is Just The Beginning}
    \framesubtitle{Model monitoring is product monitoring}
    ...
    \begin{columns}[T]
        ...
        \begin{itemize}
            \setlength\itemsep{0.4em}
            \item \textbf{Input drift:} Are queries changing?
            \item \textbf{Output quality:} Hallucination rate, refusals
            \item \textbf{Retrieval health:} Empty results, low scores
            \item \textbf{Latency:} Response time percentiles
            \item \textbf{Cost:} Tokens used, API spend
            \item \textbf{Security:} Injection attempts, data exposure
        \end{itemize}
        ...
        \begin{itemize}
            \setlength\itemsep{0.2em}
            \item User thumbs up/down
            \item Escalation to human
            \item Correction submissions
        \end{itemize}
    \end{columns}
...
\begin{frame}{Act II: Complete Summary}
